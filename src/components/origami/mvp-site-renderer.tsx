...
import type { MvpSiteSpec } from "@/lib/types";

type MvpSiteRendererProps = {
  appTitle: string;
  summary: string;
  siteSpec: MvpSiteSpec;
};

export function MvpSiteRenderer({
  appTitle,
  summary,
  siteSpec,
}: MvpSiteRendererProps) {
  return (
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative mx-auto flex max-w-7xl flex-col gap-12 px-6 py-10 lg:px-10 lg:py-14">
          ...
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {siteSpec.featureCards.map((card) => (
              <article
                key={card.title}
            ...
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em] text-[#10242c]">
                  {siteSpec.workflow.title}
                </h2>
              </div>
            </div>
            <div className="mt-8 grid gap-4 lg:grid-cols-4">
              {siteSpec.workflow.steps.map((step, index) => (
                <div
                  key={step.title}
                ...
                >
                  ...
                  <p className="mt-3 text-sm leading-6 text-[#1f2933]/68">
                    {step.description}
                  </p>
                </div>
              ))}
          ...
          <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div className="rounded-[32px] border border-[#1f2933]/8 bg-white p-6 md:p-8">
              ...
              <h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em] text-[#10242c]">
                {siteSpec.contentHighlights.title}
              </h2>
              <div className="mt-8 grid gap-4">
                {siteSpec.contentHighlights.items.map((item) => (
                  <article
                    key={item.title}
            ...
            <aside className="rounded-[32px] border border-[#1f2933]/8 bg-[#14342b] p-6 text-[#f4efe4] shadow-[0_24px_70px_rgba(20,52,43,0.18)]">
              ...
              <h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em]">
                {siteSpec.sourceProof.title}
              </h2>
              <div className="mt-6 space-y-4">
                {siteSpec.sourceProof.items.map((item) => (
                  <div
                    key={item.label}
